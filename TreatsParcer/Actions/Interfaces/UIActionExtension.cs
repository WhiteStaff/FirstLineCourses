...
namespace TreatsParcer.Actions.Interfaces
{
    ...
    {
        public static MenuItem[] ToMenuItems(this UIAction[] actions)
        {
            var items = actions.GroupBy(a => a.Category).Select(x => CreateTopMenuItem(x.Key, x.ToList())).ToArray();
            return items;
        }

        public static MenuItem ToMenuItem(this UIAction action)
        {
        ...
        public static MenuItem CreateTopMenuItem(string name, IList<UIAction> items)
        {
